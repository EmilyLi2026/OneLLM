...
/**
 * Check if a binding's daily or monthly budget is exhausted.
 * Uses spending data from validate-key response (daily_spent_cents, monthly_spent_cents).
 * Returns false if spending data is not available (backward compat).
 */
function isBindingBudgetExhausted(b: any): boolean {
  // Daily budget check
  if (b.daily_budget_cents > 0 && typeof b.daily_spent_cents === 'number') {
    if (b.daily_spent_cents >= b.daily_budget_cents) return true;
  }
  // Monthly budget check
  if (b.monthly_budget_cents > 0 && typeof b.monthly_spent_cents === 'number') {
    if (b.monthly_spent_cents >= b.monthly_budget_cents) return true;
  }
  return false;
}
...
export const requestValidator = async (c: Context, next: any) => {
  ...
  // ── OneLLM: Smart Failover Scheduling (1:N bindings) ──
  const onellmBindings = c.get('onellm_bindings');
  if (onellmBindings && onellmBindings.length > 0) {
    // Try to extract model from request body (peek at JSON body)
    ...
    } catch { /* body not yet readable or not JSON */ }

    const explicitProvider = requestHeaders[`x-${POWERED_BY}-provider`]
      || requestHeaders['x-portkey-provider'];

    // ── Filter & sort candidate bindings ──
    let candidates = [...onellmBindings].sort((a: any, b: any) => a.priority_order - b.priority_order);

    // If explicit provider specified, only consider that one
    if (explicitProvider) {
      candidates = candidates.filter((b: any) => b.provider_name === explicitProvider);
    } else if (model) {
      // Prefer bindings whose allowed_models include this model
      const modelMatches = candidates.filter(
        (b: any) => b.allowed_models && b.allowed_models.includes(model)
      );
      if (modelMatches.length > 0) {
        candidates = modelMatches;
      }
      // otherwise keep all candidates (best-effort for unknown models)
    }

    // Skip bindings with exhausted budgets (budget info comes from validate-key)
    candidates = candidates.filter((b: any) => !isBindingBudgetExhausted(b));
    ...
    if (candidates.length === 1) {
      // ── Single binding: set context for injectOnellmProvider ──
      const b = candidates[0];
      c.set('onellm_provider', {
        name: b.provider_name,
        api_key: b.api_key,
        rate_limit_rpm: b.rate_limit_rpm,
      });
      c.set('onellm_binding_id', b.id);
      // Also satisfy requestValidator's own header checks
      if (!requestHeaders[`x-${POWERED_BY}-provider`]) {
        requestHeaders[`x-${POWERED_BY}-provider`] = b.provider_name;
      }
    } else {
      // ── Multiple bindings: set fallback config for injectOnellmProvider ──
      const fallbackConfig = {
        strategy: {
          mode: 'fallback',
          onStatusCodes: [429, 500, 502, 503, 504],
        },
        targets: candidates.map((b: any) => ({
          provider: b.provider_name,
          api_key: b.api_key,
          binding_id: b.id,  // for tracking which binding served the request
        })),
      };
      c.set('onellm_fallback_config', fallbackConfig);
      // Also set provider for requestValidator's own validation check
      requestHeaders[`x-${POWERED_BY}-provider`] = candidates[0].provider_name;
      c.set('onellm_provider', {
        name: candidates[0].provider_name,
        api_key: candidates[0].api_key,
        rate_limit_rpm: candidates[0].rate_limit_rpm,
      });
    }
  }
  ...
  if (
    providerHeader &&
    !VALID_PROVIDERS.includes(providerHeader)
  ) {
    return new Response(
      JSON.stringify({
        status: 'failure',
...
export function isValidCustomHost(customHost: string, c?: Context) {
  try {
    ...
    if (value.indexOf('api.ai-hub') > -1) return false;
