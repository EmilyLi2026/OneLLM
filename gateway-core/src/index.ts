 ...
 * OneLLM Gateway
 *
 * Universal LLM Gateway with Agent Control Plane capabilities.
 *
 * @module index
 */
...
// Middlewares
import { onellmAuth } from './middlewares/onellmAuth';
...
// Agent API handlers (AI Hub extension - Phase 0)
import { registerAgent, listAgents, getAgent, getAgentCost } from './handlers/agentHandler';
 ...
 */
app.get('/', (c) => c.text('OneLLM Gateway says hey!'));

// CORS — allow browser-based clients (Playground, Chat, etc.)
app.use('*', cors({ origin: '*' }));
...
// ── Agent Management API (AI Hub Extension) ──
app.post('/api/v1/agents', registerAgent);
app.get('/api/v1/agents', listAgents);
app.get('/api/v1/agents/:id', getAgent);
app.get('/api/v1/agents/:id/cost', getAgentCost);
