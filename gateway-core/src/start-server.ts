...
if (
  ...
  const setupStaticServing = async () => {
    ...
    const serveIndex = (c: Context) => {
      if (!hasAdminTokenKey) {
        return c.html(
          'Admin token is required to access the local gateway UI. Please set admin_token in conf.json.'
        );
      }
...
async function showLoadingAnimation() {
  ...
  return new Promise((resolve) => {
    const interval = setInterval(() => {
      process.stdout.write(`\r${frames[i]} Starting OneLLM Gateway...`);
      i = (i + 1) % frames.length;
    }, 80);
...
console.log('\x1b[1m%s\x1b[0m', '🚀 OneLLM Gateway is running at:');
console.log('   ' + '\x1b[1;4;32m%s\x1b[0m', `${url}`);

// Secondary information on single lines
if (!isHeadless) {
  console.log('\n\x1b[90m📱 Console:\x1b[0m \x1b[36m%s\x1b[0m', `${url}/public/`);
}

// Single-line ready message
console.log('\n\x1b[32m✨ OneLLM Gateway ready!\x1b[0m');
