  ...
  switch (strategyMode) {
    case StrategyModes.FALLBACK:
      for (const [index, target] of currentTarget.targets.entries()) {
        ...
        const bindingId: string | undefined = (target as any).bindingId;
        const providerName: string = (target as any).provider || 'unknown';
        const ts = new Date().toISOString();

        if (index > 0) {
          // ── AI Hub: FALLOVER TRIGGERED ──
          console.log(
            `\n🔄 [AI-HUB FAILOVER] ${ts}`,
            `\n   Attempt #${index + 1}: switching to ${providerName} (binding: ${bindingId || 'N/A'})`,
            `\n   Reason: previous provider failed with status ${response?.status || 'error'}`,
            `\n`
          );
        }
        ...
        const gatewayException =
          response?.headers.get('x-onellm-gateway-exception') === 'true'
          || response?.headers.get('x-aihub-gateway-exception') === 'true';
        const isSuccess =
          (Array.isArray(codes) && !codes.includes(response?.status)) ||
          (!codes && response?.ok) ||
          gatewayException;
        if (isSuccess) {
          // ── OneLLM: inject fallback tracking headers ──
          if (bindingId || index > 0) {
            const newHeaders = new Headers(response.headers);
            if (bindingId) {
              newHeaders.set('x-onellm-binding-id', bindingId);
              newHeaders.set('x-aihub-binding-id', bindingId); // backward compat
            }
            newHeaders.set('x-onellm-fallback-attempts', String(index));
            newHeaders.set('x-aihub-fallback-attempts', String(index)); // backward compat
          ...
          if (index === 0) {
            console.log(`✅ [AI-HUB] ${ts} Primary OK: ${providerName} (binding: ${bindingId || 'N/A'}) status=${response.status}`);
          } else {
            console.log(`✅ [AI-HUB] ${ts} Failover SUCCESS: ${providerName} (binding: ${bindingId || 'N/A'}) status=${response.status} — after ${index} fallback attempt(s)`);
          }
          break;
        } else {
          console.log(`❌ [AI-HUB] ${ts} ${providerName} (binding: ${bindingId || 'N/A'}) returned ${response?.status} — ${index < currentTarget.targets.length - 1 ? 'trying next...' : 'all exhausted'}`);
        }
      }
      break;
        ...
        response = new Response(
          ...
          }),
          {
            ...
            headers: {
              ...
              'x-onellm-gateway-exception': 'true',
              'x-aihub-gateway-exception': 'true', // backward compat
            },
          }
        );
      }
...
/**
 * OneLLM: Inject bound provider API key from virtual key validation.
 * Called by all handlers to enable transparent provider key injection.
 *
 * Supports two modes (set by requestValidator middleware):
 *   1. Single binding → c.get('onellm_provider') → inject provider + api_key headers
 *   2. Multiple bindings → c.get('onellm_fallback_config') → inject x-onellm-config header (fallback strategy)
 *
 * When a request uses a OneLLM virtual key (aihub_sk_*) that is bound to
 * a Provider Credential, the onellmAuth middleware stores the decrypted
 * provider info in context. This function reads it and injects the real
 * provider API key into the request headers, so downstream handlers can
 * forward the request to the upstream model provider transparently.
 */
export function injectOnellmProvider(
  c: Context,
  headers: Record<string, any>
): void {
  // ── Multi-binding fallback config (smart failover) ──
  const fallbackConfig = c.get('onellm_fallback_config');
  if (fallbackConfig) {
    headers[`x-${POWERED_BY}-config`] = JSON.stringify(fallbackConfig);
    // Also inject the first binding's provider for validation
    const firstTarget = fallbackConfig.targets?.[0];
    if (firstTarget?.provider) {
      if (!headers[`x-${POWERED_BY}-provider`]) {
        headers[`x-${POWERED_BY}-provider`] = firstTarget.provider;
      }
      if (firstTarget.api_key) {
        headers['authorization'] = `Bearer ${firstTarget.api_key}`;
        headers[`x-${POWERED_BY}-api-key`] = firstTarget.api_key;
      }
    }
    return;
  }
  ...
  const provider = c.get('onellm_provider');
  if (provider?.api_key) {
    // Override Authorization header with the real provider API key
    // (the handler reads Authorization first, so we must replace the virtual key)
    headers['authorization'] = `Bearer ${provider.api_key}`;
    // Also set provider-specific headers for safety
    headers[`x-${POWERED_BY}-api-key`] = provider.api_key;
    if (!headers[`x-${POWERED_BY}-provider`]) {
      headers[`x-${POWERED_BY}-provider`] = provider.name;
    }
    // Apply rate limit from key config
    if (provider.rate_limit_rpm) {
      headers[`x-${POWERED_BY}-rate-limit-rpm`] = String(provider.rate_limit_rpm);
    }
  }
}
...
export function constructConfigFromRequestHeaders(
  ...
  return {
    ...
    apiKey: requestHeaders['authorization']?.replace('Bearer ', '')
      || requestHeaders[`x-${POWERED_BY}-api-key`],
