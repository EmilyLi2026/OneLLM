  ...
  if (proxyProvider === BEDROCK) {
    ...
    (async () => {
      try {
        ...
        // AI Hub: create promises for log handler to await
        // (the async IIFE runs concurrently, so we need Promises for the log handler to await)
        let resolveUsage: (v: any) => void;
        const usagePromise = new Promise(r => { resolveUsage = r; });
        if (c) c.set('streamUsagePromise', usagePromise);

        // AI Hub: capture full output content for request_logs request_output
        let outputContent = '';
