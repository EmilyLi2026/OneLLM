...
export const handler: PluginHandler = async (
  ...
  if (!apiKey || apiKey.trim() === '') {
    return {
      verdict: true,
      error:
        'AIRS_API_KEY is required but not configured. Please add your API key in the AI Hub dashboard.',
      data: null,
    };
  }
  ...
  try {
    ...
    // Extract OneLLM's trace ID from request headers to use as AIRS tr_id (AI Session ID)
    const traceId =
      ctx?.request?.headers?.['x-onellm-trace-id'] ||
      ctx?.request?.headers?.['x-aihub-trace-id'] ||
      (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function'
        ? crypto.randomUUID()
        : Math.random().toString(36).substring(2) + Date.now().toString(36));

    const payload: any = {
      tr_id: traceId, // Use AI Hub's trace ID as AIRS AI Session ID
      metadata: {
        ai_model: params.ai_model ?? 'unknown-model',
        app_user: params.app_user ?? 'aihub-gateway',
        app_name: params.app_name ? `AIHub-${params.app_name}` : 'AI Hub',
      },
      contents: [
